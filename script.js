...
const Store = (() => {
  ...
  function defaultConfig(){
    return { aluguel:0, terreno:0, alimentacaoFixa:0, energia:0, internet:0, telefone:0, financiamento:0, outrasContas:0, metaPorKm:0, metaPorHora:0 };
  }
  ...
  const RIDE_MAP = { id:'id', data:'data', hora:'hora', app:'app', valor:'valor', duracaoMin:'duracao_min', distanciaKm:'distancia_km', origem:'origem', destino:'destino', obs:'obs' };
  const CONFIG_MAP = { aluguel:'aluguel', terreno:'terreno', alimentacaoFixa:'alimentacao_fixa', energia:'energia', internet:'internet', telefone:'telefone', financiamento:'financiamento', outrasContas:'outras_contas', metaPorKm:'meta_por_km', metaPorHora:'meta_por_hora' };
  ...
  async function loadAll(){
    const [entriesRes, despesasRes, manutRes, corridasRes, configRes] = await Promise.all([
      ...
      sbClient.from('da_corridas').select('*').order('data'),
      sbClient.from('da_config').select('*').limit(1)
    ]);
    // da_corridas é uma tabela nova — se o usuário ainda não rodou o SQL dela,
    // isso não pode travar o app inteiro. Só os 4 principais são obrigatórios.
    ...
    if (corridasRes.error){
      console.warn('Tabela da_corridas ainda não existe ou não pôde ser lida — rode o schema_corridas.sql no Supabase.', corridasRes.error);
      _corridas = [];
    } else {
      _corridas = (corridasRes.data||[]).map(r => fromDb(r, RIDE_MAP));
    }
  ...
  const Corridas = makeLog('da_corridas', RIDE_MAP, () => _corridas);

  return { loadAll, getEntries, upsertEntry, deleteEntry, deleteAllEntries, getConfig, saveConfig, Despesas, Manutencoes, Corridas, defaultConfig };
})();
...
/* ============================== INSIGHTS ================================= */
const Insights = (() => {
  function generate(today, todayMetrics, allHistory){
    const list = [];
    // Só considera dias até a data visualizada (inclusive) — ver um dia antigo
    // não deve usar dias futuros como "média" de comparação.
    const historyUpTo = allHistory.filter(e => e.data <= today.data);
    const past = historyUpTo.filter(e => e.id !== today.id).map(e => ({ e, m: Calc.metrics(e) }));
    ...
    if (historyUpTo.length >= 3){
      const last7 = historyUpTo.slice(-7);
...
const UI = (() => {
  ...
  function initDashDatePicker(){
    document.getElementById('dashDatePicker').addEventListener('change', ev => {
      renderDashboard(ev.target.value);
    });
  }
  ...
  function initTheme(){
    ...
    document.getElementById('themeToggle').addEventListener('click', () => {
      ...
      try { Charts.render(Store.getEntries()); }
      catch(err){ console.error('Falha ao redesenhar os gráficos após trocar o tema:', err); }
    });
  }
  ...
  function initNav(){
    document.querySelectorAll('.nav-item').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        const titles = { dashboard:'Painel do dia', lancamento:'Novo lançamento', historico:'Histórico', despesas:'Cadastro de despesas', manutencao:'Manutenção da moto', corridas:'Corridas individuais', calculadora:'Calculadora de Corrida', comparar:'Comparação personalizada', padroes:'Padrões & Recomendações', relatorios:'Relatórios', config:'Configurações' };
        ...
        if (btn.dataset.view === 'padroes') renderPadroes();
        if (btn.dataset.view === 'calculadora') renderCalculadora();
      });
    });
  }
  ...
  function renderDashboard(dateStr){
    ...
    const autoPanel = document.getElementById('autoComparePanel');
    ...
    if (history.length === 0){
      ...
      insightList.innerHTML = '<li class="insight-empty">Registre um dia de trabalho para gerar sua primeira análise.</li>';
      autoPanel.style.display = 'none';
      if (picker) picker.value = '';
      return;
    }

    // Define qual data mostrar: a escolhida no seletor, ou por padrão o último dia lançado.
    const realTodayStr = new Date().toISOString().slice(0,10);
    if (!dateStr) dateStr = currentDashDate || history[history.length-1].data;
    currentDashDate = dateStr;
    if (picker) picker.value = dateStr;

    const today = history.find(e => e.data === dateStr);

    if (!today){
      cardsEl.innerHTML = `<p style="color:var(--text-dim)">Nenhum lançamento em ${dateStr.split('-').reverse().join('/')}. Escolha outra data ou registre esse dia em "Lançamento".</p>`;
      gauge.style.setProperty('--pct','0deg');
      gaugeValue.textContent = 'R$ 0,00';
      insightList.innerHTML = '<li class="insight-empty">Sem lançamento nesta data.</li>';
      autoPanel.style.display = 'none';
      if (hint) hint.textContent = '';
      try { Charts.render(history); }
      catch(err){ console.error('Falha ao desenhar os gráficos (não afeta seus dados salvos):', err); }
      return;
    }

    if (hint) hint.textContent = dateStr === realTodayStr ? '(hoje)' : '';
    ...
    // Comparação automática com o dia anterior — só faz sentido ao ver o dia real de hoje.
    if (dateStr === realTodayStr){
      const priorEntries = history.filter(e => e.data < dateStr);
      const prev = priorEntries.length ? priorEntries[priorEntries.length-1] : null;
      if (prev){
        renderAutoCompare(today, prev);
  ...
  /* ---------- comparação automática com o dia anterior registrado ---------- */
  function renderAutoCompare(todayEntry, prevEntry){
    const mToday = Calc.metrics(todayEntry);
    const mPrev = Calc.metrics(prevEntry);
    const dateLabel = document.getElementById('autoCompareDate');
    const grid = document.getElementById('autoCompareGrid');
    const note = document.getElementById('autoCompareNote');
    if (dateLabel) dateLabel.textContent = prevEntry.data.split('-').reverse().join('/');

    const rows = buildCompareRows(mToday, mPrev, todayEntry, prevEntry);
    grid.innerHTML = rows.map(r => `
      <div class="cmp-row">
        <span class="cmp-label">${r.label}</span>
        <span class="cmp-val">${r.prevFmt}</span>
        <span class="cmp-arrow ${r.dirClass}">${r.arrow}</span>
        <span class="cmp-val">${r.curFmt}</span>
        <span class="cmp-pct ${r.dirClass}">${r.pctLabel}</span>
      </div>`).join('');

    // Nota automática: eficiência — km cresceu muito mais que o lucro?
    const kmRow = rows.find(r => r.key === 'kmRodados');
    const lucroRow = rows.find(r => r.key === 'lucroLiquido');
    let msg = '';
    if (kmRow && lucroRow && kmRow.pct !== null && lucroRow.pct !== null){
      const diff = kmRow.pct - lucroRow.pct;
      if (diff > 15){
        msg = `Você rodou ${Calc.fmtNum(kmRow.pct,0)}% mais quilômetros, mas o lucro subiu só ${Calc.fmtNum(lucroRow.pct,0)}%. Isso indica queda na eficiência operacional em relação a ontem.`;
      } else if (lucroRow.pct - kmRow.pct > 15){
        msg = `Seu lucro cresceu bem mais que os quilômetros rodados (${Calc.fmtNum(lucroRow.pct,0)}% x ${Calc.fmtNum(kmRow.pct,0)}%) — eficiência melhorou em relação a ontem.`;
      } else {
        msg = mToday.lucroPorHora >= mPrev.lucroPorHora
          ? `Seu lucro por hora está estável ou melhor que ontem (${Calc.fmtMoney(mToday.lucroPorHora)}/h contra ${Calc.fmtMoney(mPrev.lucroPorHora)}/h).`
          : `Seu lucro por hora caiu frente a ontem (${Calc.fmtMoney(mToday.lucroPorHora)}/h contra ${Calc.fmtMoney(mPrev.lucroPorHora)}/h).`;
      }
    }
    if (note) note.textContent = msg;
  }

  // Monta as linhas de comparação entre duas métricas — reaproveitado pelo
  // comparativo automático (hoje x ontem) e pela Comparação personalizada.
  function buildCompareRows(mA, mB, entryA, entryB){
    const defs = [
      { key:'receitaBruta', label:'Receita', fmt:Calc.fmtMoney },
      { key:'lucroLiquido', label:'Lucro', fmt:Calc.fmtMoney },
      { key:'custosTotais', label:'Custos', fmt:Calc.fmtMoney, invert:true },
      { key:'lucroPorHora', label:'Lucro/hora', fmt:Calc.fmtMoney },
      { key:'receitaPorHora', label:'Valor/hora', fmt:Calc.fmtMoney },
      { key:'kmRodados', label:'KM rodados', fmt:v=>Calc.fmtNum(v,0) },
      { key:'kmL', label:'KM/L', fmt:v=>Calc.fmtNum(v) },
      { key:'corridasPorHora', label:'Corridas/hora', fmt:v=>Calc.fmtNum(v) },
      { key:'tempoCorridaMin', label:'Tempo em corrida', fmt:Calc.fmtHM },
      { key:'tempoOnlineIdleMin', label:'Aguardando chamada', fmt:Calc.fmtHM },
      { key:'tempoOfflineMin', label:'Tempo offline', fmt:Calc.fmtHM, invert:true },
      { key:'taxaOcupacao', label:'Ocupação', fmt:v=>Calc.fmtNum(v,0)+'%' },
    ];
    return defs.map(d => {
      const curVal = mA[d.key]||0, prevVal = mB[d.key]||0;
      let pct = null;
      if (prevVal !== 0) pct = (curVal-prevVal)/Math.abs(prevVal)*100;
      else if (curVal !== 0) pct = 100;
      else pct = 0;
      const threshold = 3;
      let dirClass = 'flat', arrow = '→';
      const better = d.invert ? pct < -threshold : pct > threshold;
      const worse  = d.invert ? pct > threshold : pct < -threshold;
      if (better){ dirClass = 'pos'; arrow = '▲'; }
      else if (worse){ dirClass = 'neg'; arrow = '▼'; }
      return {
        key:d.key, label:d.label, dirClass, arrow,
        curFmt:d.fmt(curVal), prevFmt:d.fmt(prevVal),
        pct, pctLabel: pct===null ? '—' : (pct>=0?'+':'')+Calc.fmtNum(pct,0)+'%'
      };
    });
  }
  ...
  function initEntryActions(){
    document.addEventListener('click', async ev => {
      ...
      try {
        ...
        if (store === 'corridas'){
          const item = Store.Corridas.getAll().find(e => e.id === id);
          if (!item) return;
          if (act === 'edit'){
            fillRideForm(item);
            document.querySelector('.nav-item[data-view="corridas"]').click();
          } else if (act === 'del'){
            if (confirm('Excluir esta corrida? Esta ação não pode ser desfeita.')){
              await Store.Corridas.remove(id);
              renderCorridas();
            }
          }
          return;
  ...
  /* ---------- comparação personalizada (duas datas quaisquer) ---------- */
  function initComparar(){
    document.getElementById('cmpBtn').addEventListener('click', renderComparar);
  }

  function renderComparar(){
    const resultEl = document.getElementById('cmpResult');
    const dateA = document.getElementById('cmpDateA').value;
    const dateB = document.getElementById('cmpDateB').value;
    if (!dateA || !dateB){
      resultEl.innerHTML = '<p style="color:var(--text-dim)">Escolha as duas datas.</p>';
      return;
    }
    const history = Store.getEntries();
    const entryA = history.find(e => e.data === dateA);
    const entryB = history.find(e => e.data === dateB);
    if (!entryA || !entryB){
      const faltando = [!entryA ? dateA : null, !entryB ? dateB : null].filter(Boolean).map(d => d.split('-').reverse().join('/'));
      resultEl.innerHTML = `<p style="color:var(--text-dim)">Não encontrei lançamento em: ${faltando.join(' e ')}.</p>`;
      return;
    }

    const mA = Calc.metrics(entryA), mB = Calc.metrics(entryB);
    const rows = buildCompareRows(mA, mB, entryA, entryB);
    const labelA = dateA.split('-').reverse().join('/');
    const labelB = dateB.split('-').reverse().join('/');
    ...
    const maisEficiente = mA.lucroPorHora >= mB.lucroPorHora ? labelA : labelB;
    const diffLucroHora = Math.abs(mA.lucroPorHora - mB.lucroPorHora);
    const conclusao = `<strong>${maisEficiente}</strong> foi o dia mais eficiente, com lucro por hora ${Calc.fmtMoney(diffLucroHora)} maior. ` +
      (mA.receitaBruta !== mB.receitaBruta ? `Receita: ${labelA} ${Calc.fmtMoney(mA.receitaBruta)} x ${labelB} ${Calc.fmtMoney(mB.receitaBruta)}. ` : '') +
      `Custos: ${labelA} ${Calc.fmtMoney(mA.custosTotais)} x ${labelB} ${Calc.fmtMoney(mB.custosTotais)}.`;

    resultEl.innerHTML = `
      <div class="compare-panel">
        <h3>${labelA} <span style="color:var(--text-dim);font-weight:400">contra</span> ${labelB}</h3>
        <div class="cmp-grid">${tableRows}</div>
      </div>
      <div class="cmp-conclusion">${conclusao}</div>
    `;
  }

  /* ---------- padrões & recomendações (estatística sobre o histórico) ---------- */
  function renderPadroes(){
    const cardsEl = document.getElementById('patternCards');
    const textEl = document.getElementById('patternText');
    const history = Store.getEntries();

    if (history.length < 3){
      cardsEl.innerHTML = '';
      textEl.innerHTML = `<p style="color:var(--text-dim)">Ainda faltam dados: com menos de 3 dias registrados não dá pra identificar padrão nenhum com confiança. Continue lançando seus dias de trabalho.</p>`;
      return;
    }

    const withMetrics = history.map(e => ({ e, m: Calc.metrics(e) }));
    const DAY_NAMES = ['Domingo','Segunda-feira','Terça-feira','Quarta-feira','Quinta-feira','Sexta-feira','Sábado'];

    // ---- melhor dia da semana (por lucro/hora médio) ----
    const porDiaSemana = {};
    withMetrics.forEach(({e,m}) => {
      const wd = new Date(e.data+'T00:00:00').getDay();
      if (!porDiaSemana[wd]) porDiaSemana[wd] = [];
      porDiaSemana[wd].push(m.lucroPorHora);
    });
    const diaSemanaStats = Object.keys(porDiaSemana).map(wd => ({
      wd:Number(wd), n:porDiaSemana[wd].length,
      avg: porDiaSemana[wd].reduce((a,b)=>a+b,0)/porDiaSemana[wd].length
    })).sort((a,b) => b.avg - a.avg);
    const melhorDia = diaSemanaStats[0];
    const melhorDiaConfiavel = melhorDia && melhorDia.n >= 2;

    // ---- melhor faixa de km rodados por dia ----
    const FAIXAS_KM = [
      { label:'até 50 km', test:v => v < 50 },
      { label:'50–100 km', test:v => v >= 50 && v < 100 },
      { label:'100–150 km', test:v => v >= 100 && v < 150 },
      { label:'150 km ou mais', test:v => v >= 150 },
    ];
    const faixaKmStats = FAIXAS_KM.map(f => {
      const itens = withMetrics.filter(({m}) => f.test(m.kmRodados));
      return { label:f.label, n:itens.length,
        avgLucroPorKm: itens.length ? itens.reduce((s,{m})=>s+m.lucroPorKm,0)/itens.length : null };
    }).filter(f => f.n > 0);
    faixaKmStats.sort((a,b) => b.avgLucroPorKm - a.avgLucroPorKm);
    const melhorFaixaKm = faixaKmStats[0];

    // ---- corridas curtas x médias x longas (km por corrida) ----
    const FAIXAS_CORRIDA = [
      { label:'curtas (até 4 km/corrida)', test:v => v < 4 },
      { label:'médias (4–8 km/corrida)', test:v => v >= 4 && v < 8 },
      { label:'longas (8 km/corrida ou mais)', test:v => v >= 8 },
    ];
    const comKmPorCorrida = withMetrics.filter(({e}) => e.corridas > 0).map(({e,m}) => ({ e, m, kmPorCorrida: m.kmRodados / e.corridas }));
    const faixaCorridaStats = FAIXAS_CORRIDA.map(f => {
      const itens = comKmPorCorrida.filter(({kmPorCorrida}) => f.test(kmPorCorrida));
      return { label:f.label, n:itens.length,
        avgLucroPorHora: itens.length ? itens.reduce((s,{m})=>s+m.lucroPorHora,0)/itens.length : null };
    }).filter(f => f.n > 0);
    faixaCorridaStats.sort((a,b) => b.avgLucroPorHora - a.avgLucroPorHora);
    const melhorFaixaCorrida = faixaCorridaStats[0];

    // cards de resumo
    const cards = [];
    if (melhorDia) cards.push({ label:'Melhor dia da semana', value: DAY_NAMES[melhorDia.wd], cls:'pos' });
    if (melhorFaixaKm) cards.push({ label:'Melhor faixa de km/dia', value: melhorFaixaKm.label, cls:'pos' });
    if (melhorFaixaCorrida) cards.push({ label:'Corridas mais rentáveis', value: melhorFaixaCorrida.label, cls:'pos' });
    cardsEl.innerHTML = cards.map(c => `<div class="stat-card"><div class="label">${c.label}</div><div class="value ${c.cls}">${c.value}</div></div>`).join('');

    // texto de recomendações
    let paragraphs = [];
    if (melhorDia){
      paragraphs.push(`<p><strong>Dia da semana:</strong> seu melhor desempenho histórico é ${DAY_NAMES[melhorDia.wd].toLowerCase()}, com lucro médio de ${Calc.fmtMoney(melhorDia.avg)}/hora` +
        (melhorDiaConfiavel ? ` (baseado em ${melhorDia.n} dias registrados nesse dia da semana).` : ` — mas isso é baseado em só ${melhorDia.n} dia registrado nesse dia da semana, ainda pouco confiável.`) + `</p>`);
    }
    if (melhorFaixaKm){
      paragraphs.push(`<p><strong>Quilometragem por dia:</strong> rodar na faixa de <strong>${melhorFaixaKm.label}</strong> tem te dado o melhor retorno por km (${Calc.fmtMoney(melhorFaixaKm.avgLucroPorKm)}/km em média, com ${melhorFaixaKm.n} dia(s) nessa faixa).</p>`);
    }
    if (melhorFaixaCorrida){
      paragraphs.push(`<p><strong>Duração das corridas:</strong> corridas <strong>${melhorFaixaCorrida.label}</strong> renderam mais por hora no seu histórico (${Calc.fmtMoney(melhorFaixaCorrida.avgLucroPorHora)}/h em média, com ${melhorFaixaCorrida.n} dia(s) nessa faixa).</p>`);
    }

    // ---- melhor horário do dia e melhor bairro/região (a partir de Corridas individuais) ----
    const corridas = Store.Corridas.getAll();
    const MIN_CORRIDAS_PADRAO = 8;
    if (corridas.length < MIN_CORRIDAS_PADRAO){
      paragraphs.push(`<div class="pattern-note">Melhor horário e melhor região ainda não têm dados suficientes — você tem ${corridas.length} corrida(s) individuais registradas, recomendo pelo menos ${MIN_CORRIDAS_PADRAO}. Registre corridas em "Corridas" (dá pra ler direto do print do Uber/99).</div>`);
    } else {
      const comHora = corridas.filter(c => c.hora && c.duracaoMin > 0);
      const FAIXAS_HORA = [
        { label:'Madrugada (0h–6h)', test:h => h>=0 && h<6 },
        { label:'Manhã (6h–12h)', test:h => h>=6 && h<12 },
        { label:'Tarde (12h–18h)', test:h => h>=12 && h<18 },
        { label:'Noite (18h–24h)', test:h => h>=18 && h<24 },
      ];
      const faixaHoraStats = FAIXAS_HORA.map(f => {
        const itens = comHora.filter(c => f.test(Number(c.hora.split(':')[0])));
        const valores = itens.map(c => c.valor / (c.duracaoMin/60));
        return { label:f.label, n:itens.length, avg: valores.length ? valores.reduce((a,b)=>a+b,0)/valores.length : null };
      }).filter(f => f.n > 0);
      faixaHoraStats.sort((a,b) => b.avg - a.avg);
      const melhorHora = faixaHoraStats[0];
      if (melhorHora){
        cardsEl.innerHTML += `<div class="stat-card"><div class="label">Melhor horário</div><div class="value pos">${melhorHora.label}</div></div>`;
        paragraphs.push(`<p><strong>Horário do dia:</strong> o período <strong>${melhorHora.label}</strong> rendeu mais por hora nas suas corridas registradas (${Calc.fmtMoney(melhorHora.avg)}/h em média, ${melhorHora.n} corrida(s)).</p>`);
      }

      const porBairro = {};
      corridas.forEach(c => {
        const bairro = extractBairro(c.destino);
        if (!bairro) return;
        if (!porBairro[bairro]) porBairro[bairro] = [];
        porBairro[bairro].push(c.valor||0);
      });
      const bairroStats = Object.keys(porBairro).map(b => ({
        bairro:b, n:porBairro[b].length, avg: porBairro[b].reduce((a,c)=>a+c,0)/porBairro[b].length
      })).filter(b => b.n >= 2).sort((a,b) => b.avg - a.avg);
      const melhorBairro = bairroStats[0];
      if (melhorBairro){
        cardsEl.innerHTML += `<div class="stat-card"><div class="label">Melhor região (destino)</div><div class="value pos">${melhorBairro.bairro}</div></div>`;
        paragraphs.push(`<p><strong>Região:</strong> corridas com destino em <strong>${melhorBairro.bairro}</strong> tiveram o maior valor médio por corrida (${Calc.fmtMoney(melhorBairro.avg)}, ${melhorBairro.n} corrida(s)) — baseado no texto do endereço, não em GPS.</p>`);
      } else {
        paragraphs.push(`<div class="pattern-note">Ainda não há bairro/região com pelo menos 2 corridas registradas para comparar com confiança.</div>`);
      }
    }

    textEl.innerHTML = paragraphs.join('');
  }
  ...
  function initConfig(){
    ...
    document.getElementById('c_metaPorKm').value = cfg.metaPorKm || 0;
    document.getElementById('c_metaPorHora').value = cfg.metaPorHora || 0;
    ...
    document.getElementById('configForm').addEventListener('submit', async ev => {
      ...
      const newCfg = {
        ...
        metaPorKm: Number(document.getElementById('c_metaPorKm').value)||0,
        metaPorHora: Number(document.getElementById('c_metaPorHora').value)||0,
      };
      try {
  ...
  /* ---------- corridas individuais (registro manual + leitura de foto) ---------- */
  let editingRideId = null;
  const RIDE_APP_LABELS = { uber:'Uber', '99':'99' };
  const MESES_PT = { 'jan':1,'fev':2,'mar':3,'abr':4,'mai':5,'jun':6,'jul':7,'ago':8,'set':9,'out':10,'nov':11,'dez':12 };

  function readRideForm(){
    const g = id => document.getElementById(id);
    return {
      id: g('rd_id').value || 'ride_' + Date.now(),
      data: g('rd_data').value,
      hora: g('rd_hora').value,
      app: g('rd_app').value,
      valor: Number(g('rd_valor').value)||0,
      duracaoMin: Number(g('rd_duracao').value)||0,
      distanciaKm: Number(g('rd_distancia').value)||0,
      origem: g('rd_origem').value,
      destino: g('rd_destino').value,
      obs: g('rd_obs').value
    };
  }
  function fillRideForm(e){
    const g = id => document.getElementById(id);
    g('rd_id').value = e.id; g('rd_data').value = e.data; g('rd_hora').value = e.hora||'';
    g('rd_app').value = e.app||'uber'; g('rd_valor').value = e.valor;
    g('rd_duracao').value = e.duracaoMin; g('rd_distancia').value = e.distanciaKm;
    g('rd_origem').value = e.origem||''; g('rd_destino').value = e.destino||''; g('rd_obs').value = e.obs||'';
    editingRideId = e.id;
    document.getElementById('rdCancelBtn').style.display = 'inline-block';
    document.getElementById('rdSubmitBtn').textContent = 'Atualizar corrida';
  }
  function clearRideForm(){
    document.getElementById('rideForm').reset();
    document.getElementById('rd_id').value = '';
    document.getElementById('rd_data').value = new Date().toISOString().slice(0,10);
    document.getElementById('rd_valor').value = 0;
    document.getElementById('rd_duracao').value = 0;
    document.getElementById('rd_distancia').value = 0;
    editingRideId = null;
    document.getElementById('rdCancelBtn').style.display = 'none';
    document.getElementById('rdSubmitBtn').textContent = 'Salvar corrida';
    document.getElementById('rdPhotoPreview').style.display = 'none';
    document.getElementById('rdPhotoInput').value = '';
    document.getElementById('rdOcrBtn').disabled = true;
    document.getElementById('rdOcrStatus').textContent = '';
  }

  function initCorridas(){
    clearRideForm();
    document.getElementById('rideForm').addEventListener('submit', async ev => {
      ev.preventDefault();
      const item = readRideForm();
      const wasEditing = !!editingRideId;
      try {
        setMsg('rdMsg', 'Salvando...', false);
        await Store.Corridas.upsert(item);
        setMsg('rdMsg', wasEditing ? 'Corrida atualizada.' : 'Corrida salva.', false);
        clearRideForm();
        renderCorridas();
      } catch(err){
        console.error(err);
        setMsg('rdMsg', 'Não foi possível salvar na nuvem. Se for a primeira corrida, confirme que rodou o schema_corridas.sql no Supabase.', true);
      }
    });
    document.getElementById('rdCancelBtn').addEventListener('click', clearRideForm);
    document.getElementById('rdSearch').addEventListener('input', renderCorridas);
  }
  ...
  function renderCorridas(){
    ...
    const list = Store.Corridas.getAll().slice().reverse().filter(e =>
      !q || e.data.includes(q) || (e.app||'').toLowerCase().includes(q) ||
      (e.origem||'').toLowerCase().includes(q) || (e.destino||'').toLowerCase().includes(q)
    );
    if (list.length === 0){
      bodyEl.innerHTML = `<tr><td colspan="7" style="color:var(--text-dim)">Nenhuma corrida registrada ainda.</td></tr>`;
      return;
    }
    bodyEl.innerHTML = list.map(e => `<tr>
      <td>${e.data.split('-').reverse().join('/')}</td>
      <td>${e.hora||'-'}</td>
      <td>${RIDE_APP_LABELS[e.app]||e.app||'-'}</td>
      <td>${Calc.fmtMoney(e.valor)}</td>
      <td>${Calc.fmtNum(e.distanciaKm)}</td>
      <td>${e.destino||'-'}</td>
      <td class="row-actions">
        <button data-store="corridas" data-act="edit" data-id="${e.id}" title="Editar">✎</button>
        <button data-store="corridas" data-act="del" data-id="${e.id}" title="Excluir">🗑</button>
      </td>
    </tr>`).join('');
  }

  /* ---------- leitura de foto (OCR client-side, sem servidor) ---------- */
  function initRidePhotoOcr(){
    const input = document.getElementById('rdPhotoInput');
    const preview = document.getElementById('rdPhotoPreview');
    const ocrBtn = document.getElementById('rdOcrBtn');
    const status = document.getElementById('rdOcrStatus');

    input.addEventListener('change', () => {
      const file = input.files[0];
      if (!file){ ocrBtn.disabled = true; preview.style.display = 'none'; return; }
    ...
    ocrBtn.addEventListener('click', async () => {
      ...
      status.textContent = 'Lendo imagem... isso pode levar alguns segundos.';
      try {
        const result = await Tesseract.recognize(file, 'por');
        const text = result.data.text || '';
        const parsed = parseRideText(text);
        applyParsedRide(parsed);
        status.textContent = 'Leitura concluída — confira e corrija os campos abaixo antes de salvar.';
      } catch(err){
        console.error(err);
        status.style.color = 'var(--danger)';
        status.textContent = 'Não consegui ler essa imagem. Preencha os campos manualmente.';
      } finally {
        ocrBtn.disabled = false;
      }
    });
  ...
  function applyParsedRide(p){
    const g = id => document.getElementById(id);
    if (p.data) g('rd_data').value = p.data;
    if (p.hora) g('rd_hora').value = p.hora;
    if (p.valor != null) g('rd_valor').value = p.valor;
    if (p.duracaoMin != null) g('rd_duracao').value = p.duracaoMin;
    if (p.distanciaKm != null) g('rd_distancia').value = p.distanciaKm;
    if (p.origem) g('rd_origem').value = p.origem;
    if (p.destino) g('rd_destino').value = p.destino;
  }

  // Extrai o que der de estruturado do texto reconhecido no print da corrida.
  // Cada campo é independente — se não achar um, simplesmente deixa em branco
  // (o formulário sempre fica editável, nunca confia cegamente na leitura).
  function parseRideText(text){
    const out = {};

    const valorMatch = text.match(/R\$\s*([\d]{1,3}(?:\.\d{3})*,\d{2})/);
    if (valorMatch) out.valor = Number(valorMatch[1].replace(/\./g,'').replace(',', '.'));

    const kmMatch = text.match(/([\d]+(?:[.,]\d+)?)\s*km/i);
    if (kmMatch) out.distanciaKm = Number(kmMatch[1].replace(',', '.'));

    const minMatch = text.match(/(\d+)\s*min/i);
    const segMatch = text.match(/(\d+)\s*segundo/i);
    if (minMatch){
      let mins = Number(minMatch[1]);
      if (segMatch) mins += Number(segMatch[1])/60;
      out.duracaoMin = Math.round(mins*10)/10;
    }

    const dataMatch = text.match(/(\d{1,2})\s*de\s*([a-zçã]{3,4})\.?\s*de\s*(\d{4})/i);
    if (dataMatch){
      const mesNum = MESES_PT[dataMatch[2].toLowerCase().slice(0,3)];
      if (mesNum){
        const dia = String(dataMatch[1]).padStart(2,'0');
        const mes = String(mesNum).padStart(2,'0');
        out.data = `${dataMatch[3]}-${mes}-${dia}`;
      }
    }

    const horaMatch = text.match(/\b([01]?\d|2[0-3]):([0-5]\d)\b/);
    if (horaMatch) out.hora = `${horaMatch[1].padStart(2,'0')}:${horaMatch[2]}`;

    // Endereços: o app quebra cada endereço em 2 linhas na tela ("...São" \n "Raimundo - São Luís - MA, 65057-776, BR").
    // Sem juntar as duas, a leitura pega só o final do endereço. Aqui: sempre que uma
    // linha tem vírgula mas não termina em CEP+BR, e a linha seguinte completa com
    // CEP+BR, as duas são juntadas como um endereço só.
    const linhas = text.split('\n').map(l => l.trim()).filter(Boolean);
    const CEP_BR = /\d{5}-?\d{3}.*BR\.?$/i;
    const enderecos = [];
    for (let i = 0; i < linhas.length; i++){
      const linha = linhas[i];
      if (CEP_BR.test(linha)){
        enderecos.push(linha);
      } else if (i+1 < linhas.length && CEP_BR.test(linhas[i+1]) && linha.includes(',')){
        enderecos.push(linha + ' ' + linhas[i+1]);
        i++; // já usou a próxima linha, pula ela
      }
    }
    if (enderecos[0]) out.origem = enderecos[0];
    if (enderecos[1]) out.destino = enderecos[1];

    return out;
  }

  /* ---------- calculadora de corrida (aceitar/recusar antes de aceitar a chamada) ---------- */
  function initCalculadora(){
    ['cl_valor','cl_distancia','cl_tempo'].forEach(id => {
      document.getElementById(id).addEventListener('input', renderCalculadora);
    });
    document.getElementById('calcClearBtn').addEventListener('click', () => {
      ['cl_valor','cl_distancia','cl_tempo'].forEach(id => { document.getElementById(id).value = ''; });
      renderCalculadora();
    });
    document.getElementById('calcRegisterBtn').addEventListener('click', () => {
      const valor = Number(document.getElementById('cl_valor').value)||0;
      const distancia = Number(document.getElementById('cl_distancia').value)||0;
      const tempo = Number(document.getElementById('cl_tempo').value)||0;
      clearRideForm();
      document.getElementById('rd_data').value = new Date().toISOString().slice(0,10);
      document.getElementById('rd_hora').value = new Date().toTimeString().slice(0,5);
      document.getElementById('rd_valor').value = valor;
      document.getElementById('rd_distancia').value = distancia;
      document.getElementById('rd_duracao').value = tempo;
      document.querySelector('.nav-item[data-view="corridas"]').click();
      setMsg('rdMsg', 'Dados pré-preenchidos a partir da calculadora — confira origem/destino e salve.', false);
    });
    renderCalculadora();
  }

  function renderCalculadora(){
    const semaforo = document.getElementById('calcSemaforo');
    const detail = document.getElementById('calcDetail');
    const hint = document.getElementById('calcMetaHint');
    const registerBtn = document.getElementById('calcRegisterBtn');
    if (!semaforo) return;
    ...
    const tempo = Number(document.getElementById('cl_tempo').value)||0;
    const cfg = Store.getConfig();
    const metaKm = cfg.metaPorKm||0, metaHora = cfg.metaPorHora||0;

    if (metaKm>0 || metaHora>0){
      const partes = [];
      if (metaKm>0) partes.push(Calc.fmtMoney(metaKm)+'/km');
      if (metaHora>0) partes.push(Calc.fmtMoney(metaHora)+'/hora');
      hint.textContent = `Sua meta: ${partes.join(' · ')} — ajustar em Configurações.`;
    } else {
      hint.textContent = 'Você ainda não definiu meta em Configurações — o resultado abaixo só mostra os números, sem veredito verde/amarelo/vermelho.';
    }

    if (valor<=0 || (distancia<=0 && tempo<=0)){
      semaforo.className = 'calc-semaforo';
      semaforo.textContent = '?';
      detail.textContent = 'Preencha o valor e ao menos a distância ou o tempo.';
      registerBtn.disabled = true;
      return;
    }

    const valorPorKm = distancia>0 ? valor/distancia : null;
    const valorPorHora = tempo>0 ? valor/(tempo/60) : null;

    const linhas = [];
    if (valorPorKm!=null) linhas.push(`${Calc.fmtMoney(valorPorKm)}/km`);
    if (valorPorHora!=null) linhas.push(`${Calc.fmtMoney(valorPorHora)}/hora`);

    let okKm = null, okHora = null;
    if (metaKm>0 && valorPorKm!=null) okKm = valorPorKm >= metaKm;
    if (metaHora>0 && valorPorHora!=null) okHora = valorPorHora >= metaHora;
    const avaliacoes = [okKm, okHora].filter(v => v !== null);

    let cor = '', texto = '—';
    if (avaliacoes.length === 0){
      cor = ''; texto = '—';
    } else if (avaliacoes.every(v => v === true)){
      cor = 'verde'; texto = '✓';
    } else if (avaliacoes.every(v => v === false)){
      cor = 'vermelho'; texto = '✗';
    } else {
      cor = 'amarelo'; texto = '!';
    }
    semaforo.className = 'calc-semaforo ' + cor;
    semaforo.textContent = texto;

    let msg = linhas.join(' · ');
    if (cor === 'verde') msg += ' — dentro da sua meta, vale aceitar.';
    else if (cor === 'vermelho') msg += ' — abaixo da sua meta, considere recusar.';
    else if (cor === 'amarelo') msg += ' — bate a meta em um critério, mas não no outro.';
...
const Auth = (() => {
  ...
  async function bootApp(){
    try {
      ...
      showApp();
      UI.init();
      // Atalho do app (ex: "Aceitar corrida?") abre direto na tela pedida via ?view=...
