...
const LS_DESPESAS = 'da_despesas_v1';
const LS_MANUT    = 'da_manutencoes_v1';
...
const Store = (() => {
  ...
  // Genérico: usado tanto para Despesas quanto para Manutenção — mesma forma de
  // ler, gravar, inserir/atualizar e apagar, só muda a chave do localStorage.
  function makeLog(key){
    function getAll(){
    ...
    function remove(id){ saveAll(getAll().filter(i => i.id !== id)); }
    return { getAll, saveAll, upsert, remove };
  }
  const Despesas = makeLog(LS_DESPESAS);
  const Manutencoes = makeLog(LS_MANUT);
  return { getEntries, saveEntries, upsertEntry, deleteEntry, getConfig, saveConfig, Despesas, Manutencoes };
})();
...
const UI = (() => {
  ...
  function initDespesas(){
    clearDespesaForm();
    document.getElementById('despesaForm').addEventListener('submit', ev => {
      ev.preventDefault();
      const item = readDespesaForm();
      const wasEditing = !!editingDespesaId;
      Store.Despesas.upsert(item);
  ...
  function initManutencao(){
    clearManutForm();
    document.getElementById('manutForm').addEventListener('submit', ev => {
      ev.preventDefault();
      const item = readManutForm();
      const wasEditing = !!editingManutId;
      Store.Manutencoes.upsert(item);
